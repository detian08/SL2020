odoo.define('custom_sale_pricelist_calulation.models_extend', function (require) {
"use strict";
...
models.PosModel = models.PosModel.extend({
 initialize: function (session, attributes) {
        ...
        fields: ['display_name', 'list_price', 'lst_price', 'standard_price', 'categ_id', 'pos_categ_id', 'taxes_id',
                 ...
                 'product_tmpl_id','tracking', 'second_sale_price'],
        ...
        _.find(pricelist_items, function (rule) {
            ...
            if (rule.compute_price === 'fixed') {
                price = rule.fixed_price;
                 ...
                 if (rule.second_sale_price === true) {
                    price = self.second_sale_price - (self.second_sale_price * (rule.percent_price / 100));
                 }
                return true;
            } else {
                var price_limit = price;
                if (rule.second_sale_price === true) {
                       var price_limit = self.second_sale_price
                }
                price = price - (price * (rule.price_discount / 100));
                if (rule.second_sale_price === true) {
                    price = self.second_sale_price - (self.second_sale_price * (rule.price_discount / 100));
                }
                if (rule.price_round) {
